...
var sendInfoToDB1 = function (request, response, next) {
  ...
  pg.connect(process.env.DATABASE_URL, function(err, client, done) {
    client.query('INSERT INTO url_short_microservice (original_url, short_url) WHERE NOT EXISTS ( SELECT original_url FROM url_short_microservice WHERE original_url ='+mysqlOrignalHttpForUse+' VALUES ('+mysqlOrignalHttpForUse+', "test"))', function(err, result) {
      if (err)
       //{ resultsSQL = "Error "+ err; response.send("Error " + err);  }
	   { resultsSQL = ("Error " + err); }
      else
...
var sendInfoToDB2 = function (request, response){
  var OrignalHttpForUse = (request.url).substring(5);
 var mysqlOrignalHttpForUse = OrignalHttpForUse.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/"/g, '&quot;');
